...
function LoanDetail() {
    ...
    const generateApplicationPDF = () => {
        ...
        autoTable(doc, {
            startY: y,
            head: [["Due Date", "Principal", "Interest", "Fees", "Total Due", "Balance After"]],
            body: schedule.map(s => [
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            <h1 className="text-lg font-bold text-gray-900">{loan.product} Loan</h1>
                            <Badge variant={loan.status === 'Active' ? 'default' : 'secondary'}
                                className={loan.status === 'Active' ? 'bg-[#174271]' : ''}>
                                {loan.status}
                            </Badge>
                        </div>
                    ...
                    <div className="flex gap-3">
                        <Button onClick={generateApplicationPDF} variant="outline" className="border-[#174271] text-[#174271] hover:bg-[#174271]/10">
                            Download Schedule
                        </Button>
                        <Button className="bg-[#174271] hover:bg-[#034625]">
                            Make Repayment
                        </Button>
                    </div>
                </div>
                    ...
                    {['overview', 'schedule', 'transactions'].map((tab) => (
                        <button
                            ...
                            className={`
                                ...
                                ${activeTab === tab
                                    ? 'bg-white text-[#174271] shadow'
                                    : 'text-gray-600 hover:bg-white/[0.12] hover:text-gray-800'}
                            `}
                        >
                            {tab.charAt(0).to() + tab.slice(1)}
                        </button>
                    ))}
                </div>
                ...
                <div className="space-y-6">
                    {activeTab === 'overview' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <Card>
                                <CardHeader>
                                    <CardTitle className="text-[#174271]">Loan Overview</CardTitle>
                                </CardHeader>
                            ...
                            <div className="space-y-6">
                                <Card>
                                    <CardHeader>
                                        <CardTitle className="text-[#174271]">Terms & Dates</CardTitle>
                                    </CardHeader>
                                    <CardContent className="space-y-4">
                                        <div className="flex justify-between py-2 border-b">
                                ...
                                <Card className="border-[#D4AF37] bg-[#D4AF37]/30 overflow-hidden shadow-sm">
                                    <CardHeader className="bg-[#D4AF37]/50 pb-3">
                                    ...
                                    <CardContent className="space-y-3 pt-4">
                                        {isPayoffLoading ? (
                                            <div className="py-4 text-center text-sm text-[#D4AF37] animate-pulse">
                                            ...
                                            <div className="py-4 text-center text-xs text-[#D4AF37] italic">
                                                Unable to generate payoff quote
                                            </div>
                                        )}
                                    </CardContent>
                                    <CardFooter className="bg-[#D4AF37] px-6 py-3 border-t border-[#D4AF37] flex items-start gap-2">
                                        <Info className="h-4 w-4 text-[#D4AF37] shrink-0 mt-0.5" />
                                        <p className="text-[10px] leading-tight text-[#D4AF37]/60 font-medium">
                                    ...
                                    <TableBody>
                                        {schedule.map((row, i) => (
                                            <TableRow key={i}>
                                                ...
                                                <TableCell>{formatCurrency(row.fee_due)}</TableCell>
                                                <TableCell className="font-bold text-[#174271]">{formatCurrency(row.total_due)}</TableCell>
                                            ...
                                            <TableRow key={i}>
                                                ...
                                                <TableCell>
                                                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium 
                                                        ${t.type === 'Disbursement' ? 'bg-blue-100 text-blue-800' : 'bg-[#D4AF37] text-[#D4AF37]'}`}>
                                                        {t.type}
                                                    </span>
                                                </TableCell>
