...
function BulkLoanRepaymentsUpload({ isOpen, onClose, refetchTransactions }) {
  ...
  const handleBulkUpdate = async (values, { resetForm }) => {
    ...
    try {
      ...
      toast.error("Failed to upload bulk loan repayments. Please try again.");
    } finally {
      setLoading(false);
    }
  };
              ...
              <div className="space-y-4 py-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-2">
                    CSV should include columns for each loan type, e.g.,{" "}
                    <code>&lt;Loan Type&gt; Account</code>,{" "}
                    <code>&lt;Loan Type&gt; Repayment Amount</code> (like
                    "Personal Loan Account", "Personal Loan Repayment Amount").
                    Optional: <code>Payment Method</code> (defaults to "Cash"),{" "}
                    <code>Repayment Type</code> (defaults to "Regular
                    Repayment").{" "}
...
export default BulkLoanRepaymentsUpload;
