...
function BulkCombinedUpload({ isOpen, onClose, refetchTransactions }) {
  ...
  const handleBulkUpdate = async (values, { resetForm }) => {
    ...
    try {
      ...
      toast.success("Bulk combined transactions uploaded successfully");
      resetForm();
      onClose();
      refetchTransactions();
    } catch (error) {
      toast.error(
        "Failed to upload bulk combined transactions. Please try again."
      );
    } finally {
      setLoading(false);
    }
              ...
              <div className="space-y-4 py-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-2">
                    ...
                    deposits, and venture payments, e.g.,{" "}
                    <code>&lt;Savings Type&gt; Account</code>,{" "}
                    <code>&lt;Savings Type&gt; Amount</code>,{" "}
                    <code>&lt;Venture Type&gt; Account</code>,{" "}
                    <code>&lt;Venture Type&gt; Amount</code>,{" "}
                    <code>&lt;Venture Type&gt; Payment Amount</code>. Optional:{" "}
...
export default BulkCombinedUpload;
